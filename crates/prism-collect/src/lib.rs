...
use prism_core::{Device, PrismError, Query, RankedResult, Result, ResultType, Snapshot};
...
fn sample_results_for(q: &str) -> Vec<RankedResult> {
    ...
    for (i, (domain, title)) in domains.iter().enumerate() {
        ...
        results.push(RankedResult {
            ...
            result_type: if i == 0 { ResultType::FeaturedSnippet } else { ResultType::Organic },
            has_sitelinks: i % 3 == 0,
...
pub fn provider_by_name(name: &str) -> Result<Box<dyn SerpProvider>> {
    match name {
        ...
        other => Err(PrismError::Provider(format!(
            "unknown or disabled provider: {}. only public permitted sources are supported", other
        ))),
    }
}
...
mod tests {
    ...
    async fn sample_capture_returns_results() {
        ...
        let q = Query { text: "site architecture".into(), locale: Some("en-US".into()), device: Device::Desktop };
